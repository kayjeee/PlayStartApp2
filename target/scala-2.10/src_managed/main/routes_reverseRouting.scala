...
def confirm(confirmToken:String): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "confirm/" + implicitly[PathBindable[String]].unbind("confirmToken", confirmToken))
}
...
def validateEmail(token:String): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "email/" + implicitly[PathBindable[String]].unbind("token", token))
}
...
def confirm : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      ...
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "confirm/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("confirmToken", confirmToken)})
      }
   """
)
...
def validateEmail : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      function(token) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "email/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("token", token)})
      }
   """
)
...
def runReset(token:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Reset.runReset(token), HandlerDef(this, "controllers.account.Reset", "runReset", Seq(classOf[String]), "POST", """""", _prefix + """reset/$token<[^/]+>""")
)
                      

// @LINE:26
def ask(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Reset.ask(), HandlerDef(this, "controllers.account.Reset", "ask", Seq(), "GET", """ Displays a page to send a reset email given the user's email.""", _prefix + """reset/ask""")
)
                      

// @LINE:30
def reset(token:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Reset.reset(token), HandlerDef(this, "controllers.account.Reset", "reset", Seq(classOf[String]), "GET", """ Resets the password using the password.""", _prefix + """reset/$token<[^/]+>""")
)
                      

// @LINE:27
def runAsk(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Reset.runAsk(), HandlerDef(this, "controllers.account.Reset", "runAsk", Seq(), "POST", """""", _prefix + """reset/ask""")
)
...
def save(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Signup.save(), HandlerDef(this, "controllers.account.Signup", "save", Seq(), "POST", """""", _prefix + """signup""")
)
                      

// @LINE:23
def confirm(confirmToken:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Signup.confirm(confirmToken), HandlerDef(this, "controllers.account.Signup", "confirm", Seq(classOf[String]), "GET", """ Accessed when the user confirms the registration.""", _prefix + """confirm/$confirmToken<[^/]+>""")
)
                      

// @LINE:19
def create(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.Signup.create(), HandlerDef(this, "controllers.account.Signup", "create", Seq(), "GET", """ Registers the user, sending an email to confirm the account.""", _prefix + """signup""")
)
...
def runPassword(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Password.runPassword(), HandlerDef(this, "controllers.account.settings.Password", "runPassword", Seq(), "POST", """""", _prefix + """settings/password""")
)
                      

// @LINE:13
def index(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Password.index(), HandlerDef(this, "controllers.account.settings.Password", "index", Seq(), "GET", """""", _prefix + """settings/password""")
)
...
// @LINE:12
def index(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Index.index(), HandlerDef(this, "controllers.account.settings.Index", "index", Seq(), "GET", """""", _prefix + """settings""")
)
...
def validateEmail(token:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Email.validateEmail(token), HandlerDef(this, "controllers.account.settings.Email", "validateEmail", Seq(classOf[String]), "GET", """""", _prefix + """email/$token<[^/]+>""")
)
                      

// @LINE:16
def runEmail(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Email.runEmail(), HandlerDef(this, "controllers.account.settings.Email", "runEmail", Seq(), "POST", """""", _prefix + """settings/email""")
)
                      

// @LINE:15
def index(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.account.settings.Email.index(), HandlerDef(this, "controllers.account.settings.Email", "index", Seq(), "GET", """""", _prefix + """settings/email""")
)
